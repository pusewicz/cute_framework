...
Returns an opaque [CF_Threadpool](/multithreading/cf_threadpool) pointer.
...
Call [cf_destroy_threadpool](/multithreading/cf_destroy_threadpool) when done. A threadpool manages a set of threads. Each thread sleeps until a task is placed
into the threadpool (see: [CF_TaskFn](/multithreading/cf_taskfn)). Once the task is completed, the thread attempts to fetch another task. If no more
