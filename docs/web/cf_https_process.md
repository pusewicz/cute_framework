---
title: cf_https_process
parent: web
---
[//]: # (This file is automatically generated by Cute Framework's docs parser.)
[//]: # (Do not edit this file by hand!)
[//]: # (See: https://github.com/RandyGaul/cute_framework/blob/master/samples/docs_parser.cpp)
# cf_https_process

GitHub: [cute_https.h](https://github.com/RandyGaul/cute_framework/blob/master/include/cute_https.h)
Processes a request and generates a response.

```cpp
CF_HttpsResult cf_https_process(CF_HttpsRequest request);
```

## Return Value

Returns the status of the request [CF_HttpsResult](/web/cf_httpsresult).

## Remarks

You should call this function in a loop. See [CF_HttpsResult](/web/cf_httpsresult).

## Related Pages

  * [CF_HttpsResult](/web/cf_httpsresult)  
  * [cf_https_response](/web/cf_https_response)  
  * [cf_https_get](/web/cf_https_get)  
  * [cf_https_post](/web/cf_https_post)  
