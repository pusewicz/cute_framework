...
// a real display-density change would. Pass 0 to clear the override and re-query SDL. Coerced to
// 1.0 under CF_APP_OPTIONS_NO_HIGH_DPI_BIT (that bit pins the production value); NaN and negatives
// clear. This exists because CI and most dev machines run at pixel_scale 1.0, where logical points
// and physical pixels are the same number and points-vs-pixels mixups are invisible; see
...
struct CF_App
{
	...
	float pixel_scale_override = 0; // Nonzero pins pixel_scale to this value (cf_app_force_pixel_scale test/debug hook); density events stop re-querying SDL while set.
